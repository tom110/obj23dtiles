...
function loadObj(objPath, options) {
    ...
    function addFace(vertices, positions, uvs, normals) {
        // 修改索引，其实就是根据face索引全部减一，负数的情况暂时没有看懂
        ...
        checkPrimitive(uvs, faceNormals);//检查贴图和法线的合法性
    ...
    function parseLine(line) {
        ...
        if ((line.length === 0) || (line.charAt(0) === '#')) {
            ...
            if (Cartesian3.equals(normal, Cartesian3.ZERO)) { //如果三维向量为零向量
                Cartesian3.clone(Cartesian3.UNIT_Z, normal); //让法线向量为z轴单位向量
            } else {
                Cartesian3.normalize(normal, normal);//矩阵标准化
            }
            if (defined(axisTransform)) {
                Matrix4.multiplyByPointAsVector(axisTransform, normal, normal); // 如果输入和输出坐标系不一致，则转换
            }
            //处理后的法线坐标压入全局法线数组
