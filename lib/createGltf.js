...
function addCombinedBuffers(gltf, bufferState, name) {
    ...
    addCombinedBufferView(gltf, bufferState.batchIdBuffers, bufferState.batchIdAccessors, 0, WebGLConstants.ARRAY_BUFFER);
...
function addPrimitive(
    ...
    const hasNormals = primitive.normals.length > 0;
    const hasUVs = primitive.uvs.length > 0;
    const useBatchId = options.batchId;
    ...
    if (hasPositions) {
        ...
        bufferState.batchTableJson.maxPoint.push(
            gltf.accessors[accessorIndex].max
        );
        bufferState.batchTableJson.minPoint.push(
            gltf.accessors[accessorIndex].min
        );
    }
    let accessorIndex;
    if (hasNormals) {
        accessorIndex = addVertexAttribute(
            gltf,
            primitive.normals,
            3,
    ...
    if (useBatchId) {
        const batchIdCount = primitive.positions.length / 3;
        accessorIndex = addBacthIdAttribute(
            gltf,
            bufferState.currentBatchId,
            batchIdCount,
            primitive.name + '_batchId'
        );
        attributes._BATCHID = accessorIndex;
        const batchIdArray = new ArrayStorage(ComponentDatatype.UNSIGNED_SHORT);
        for (let j = 0; j < batchIdCount; j++) {
