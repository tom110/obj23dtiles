...
const combine = require('../lib/combineTileset');
const obj23dtiles = require('../lib/obj23dtiles');

const defined = Cesium.defined;

const defaults = obj23dtiles.defaults;
...
const argv = yargs
    ...
    .options({
        ...
        b3dm: {
            describe:
                'Convert to b3dm model file, with _BATCHID and default batch table per-mesh.',
            type: 'boolean',
            default: defaults.b3dm
        },
        i2dm: {
            describe:
                'Convert to i3dm model file, with custom FeatureTable and BatchTable.',
            type: 'boolean',
...
if (
    argv.metallicRoughness + argv.specularGlossiness + argv.materialsCommon >
    1
) {
    console.error(
        'Only one material type may be set from [--metallicRoughness, --specularGlossiness, --materialsCommon].'
    );
    process.exit(1);
}
...
let extension = path.extname(outputPath).toLowerCase();
